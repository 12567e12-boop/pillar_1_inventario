...
async def seleccionar_supervisor_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, supervisor: str):
    ...
    # Enviar nuevo mensaje para la fecha con la fecha actual dinámica
    await update.effective_chat.send_message(
...
async def seleccionar_especialidad_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, especialidad: str):
    ...
    # Establecer el estado para esperar la imagen
    UserSession.set_user_state(context, "esperando_imagen")

    # Enviar mensaje pidiendo la imagen (sin crear aún la requisición en DB)
    mensaje = (
        f"✅ Especialidad seleccionada: <b>{especialidad}</b>\n\n"
        "Ahora, por favor adjunta una imagen de la requisición (o escribe /skip para omitir).\n\n"
        "La requisición se registrará cuando se procese la imagen."
    )
...
async def handle_esperando_especialidad(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    """
    Maneja el paso de ingresar la especialidad y pasa al siguiente paso (imagen).

    Args:
        update: Update de Telegram
        context: Contexto de la conversación
        text: Especialidad requerida
    """
    temp_req = UserSession.get_temp_requisicion(context)
    ...
    await update.message.reply_text(
        Messages.NUEVA_REQUISICION_IMAGEN,
        parse_mode='Markdown',
