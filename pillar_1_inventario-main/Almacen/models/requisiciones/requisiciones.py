...
class Requisicion(models.Model):
    ...
    # --- Detalles de Productos (hasta 14) ---
    def validar_max_productos(value):
        if len(value) > 14:
            raise ValidationError('No se pueden agregar más de 14 productos a la requisición')
    
    detalles_productos = models.JSONField(
        default=list,
        help_text="Lista de hasta 14 productos con sus detalles",
        validators=[validar_max_productos]
    )
    ...
    def get_absolute_url(self):
        ...
        return reverse('requisiciones:detalle_requisicion', kwargs={'token_publico': str(self.token_publico)})

    def agregar_producto(self, producto_id, cantidad, unidad_id, descripcion=''):
        """
        Agrega un producto a la lista de detalles
        """
        if len(self.detalles_productos) >= 14:
            raise ValueError("No se pueden agregar más de 14 productos a la requisición")
            
        producto = Producto.objects.get(id=producto_id)
        unidad = Unidad.objects.get(id=unidad_id)
        
        self.detalles_productos.append({
    ...
    def obtener_productos(self):
        """
        Devuelve la lista de productos con objetos completos
        """
        detalles = []
        for detalle in self.detalles_productos:
            try:
                producto = Producto.objects.get(id=detalle['producto_id'])
                unidad = Unidad.objects.get(id=detalle['unidad_id'])
                detalles.append({
                    'producto': producto,
            ...
            except (Producto.DoesNotExist, Unidad.DoesNotExist, KeyError):
                continue
        return detalles
